...
declare variable $tests := xquery:eval(unparsed-text('tests.xml'));
...
declare function local:parser-test( $template, $parseTree ) {
  xquery:eval( unparsed-text('run-parser.xqy'), map { 'template': $template, 'parseTree': $parseTree }  ), map {'pass': true()} };

declare function local:compiler-test( $template, $hash, $output ) {
    xquery:eval( unparsed-text('run-compiler.xqy'), map { 'template': $template, 'hash': $hash,
      'output': $output}, map {'pass': true()} )
};
...
return <test position="{$i}" parseTest="{if($valid) then 'ok' else 'NOK'}">
       ...
       </test> } catch * { <test type="ERROR" i="{$i}"  parseTest="NOK" compileTest="NOK">{
         $test/@name}
         <stackTrace>{$err:additional}</stackTrace>
         </test> }
} </tests>
return <result>
